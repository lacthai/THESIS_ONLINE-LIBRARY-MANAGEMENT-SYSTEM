      ...
      {navbarLinksNotAuthenticated.map((map_para, index) => {
        ...
        return (
          <Link to={url} className="nav-link flex justify-center  h-full mx-2">
            <div
              className="flex items-center  justify-center dark:text-[#303030] text-[#e0e0e0]  bg-[#6870fa] dark:bg-[#4cceac] w-[80px] rounded-full border-[2px] border-[#e0e0e0] dark:border-[#303030] hover:scale-[1.05] transition-[0.3s]"
              key={index}
            >
              {name}
            </div>
